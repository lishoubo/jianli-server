...
public abstract class CouchDBStore<T extends BaseModel> {
    ...
    public Result<Response> save(T model) {
        try {
            ...
            if (response.getError() != null) {
                logger.error("[couchdb][save] response not success.{}", JSON.toJSONString(response));
                return Result.create(StatusCode.SERVER_SAVE_FAIL);
            }
            return Result.create(response);
        } catch (Exception e) {
            logger.error("[couchdb][save] exception.{}", JSON.toJSONString(model), e);
        }
    ...
    protected Result<Pagination<T>> pageQuery0(String viewId, PageQuery pageQuery, Class<T> clazz, boolean includeDocs) {
        View view = couchDbClient.view(viewId);
        try {
            Page<T> tPage = view.includeDocs(includeDocs).skip(pageQuery.getOffset()).queryPage(pageQuery.getPageSize(), null, clazz);
            return Result.pagination(tPage.getResultList(),
                    pageQuery,
                    (int) (tPage.getTotalResults() / pageQuery.getPageSize()) + 1);
        } catch (Exception e) {
    ...
    protected List<T> listQuery0(String viewId, Class<T> clazz, int limit, Object... keys) {
        View view = couchDbClient.view(viewId);
        try {
            List<T> result = view.includeDocs(true).key(keys).limit(limit).query(clazz);
            return result;
        } catch (Exception e) {
            logger.error("[couchbb][query] exception.", e);
        }
