...
import com.jl.platform.dao.model.BaseModel;
...
public abstract class MongoDBStore<T extends BaseModel> implements InitializingBean {
    ...
    public Result<Pagination<T>> pageQuery(PageQuery pageQuery) {
        try {
            FindIterable<Document> iterable =
                    collection.find()
                            .sort(pageQuery.isDesc() ? Sorts.descending(pageQuery.getSortFiled()) : Sorts.ascending(pageQuery.getSortFiled()))
    ...
    public Result count() {
        return Result.create(collection.count());
    }
